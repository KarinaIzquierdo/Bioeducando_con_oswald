...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        .current-img { width: 100%; max-height: 200px; object-fit: cover; border-radius: 15px; margin-bottom: 10px; }
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar" style="width: 100%; box-sizing: border-box;">
            <h2><i data-lucide="newspaper"></i> Editar Noticia</h2>
            ...
            <div class="form-card">
                <form action="{{ route('admin.noticias.update', $noticia->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="antetitulo" class="form-control" value="{{ old('antetitulo', $noticia->antetitulo) }}" placeholder="Contextualiza el tema">
                    </div>
                    <div class="form-group">
                        <label>Titulo <span style="color:#b91c1c">*</span></label>
                        <input type="text" name="titulo" class="form-control" value="{{ old('titulo', $noticia->titulo) }}" required placeholder="El encabezado principal de la noticia">
                    </div>
                    <div class="form-group">
                        <label>Subtitulo <span class="opt">(opcional)</span></label>
                        <textarea name="subtitulo" class="form-control" placeholder="Amplia la información del título">{{ old('subtitulo', $noticia->subtitulo) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="entradilla" class="form-control" required placeholder="Primer párrafo de introducción">{{ old('entradilla', $noticia->entradilla) }}</textarea>
                    </div>
                    <div class="form-group">
                        <label>Cuerpo de la noticia <span style="color:#b91c1c">*</span></label>
                        <textarea name="cuerpo" class="form-control" rows="10" required placeholder="Escribe aquí el cuerpo completo de la noticia">{{ old('cuerpo', $noticia->cuerpo) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($noticia->imagen)
                            @php
                                $ext = pathinfo($noticia->imagen, PATHINFO_EXTENSION);
                                $isImg = in_array(strtolower($ext), ['jpg','jpeg','png','gif','webp']);
                                $isVideo = in_array(strtolower($ext), ['mp4','mov','avi']);
                            @endphp
                            @if($isImg)
                                <img src="{{ asset('storage/' . $noticia->imagen) }}" class="current-img" alt="Archivo actual">
                            @elseif($isVideo)
                                <video src="{{ asset('storage/' . $noticia->imagen) }}" class="current-img" controls></video>
                            @else
                                <div class="current-img" style="background:#f0f0f0;display:flex;align-items:center;justify-content:center;gap:10px;"><i data-lucide="file-text" size="32" style="color:#744d2d;"></i><span style="color:#444;font-weight:600;">PDF: {{ $noticia->imagen }}</span></div>
                            @endif
                        @endif
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="pie_foto" class="form-control" value="{{ old('pie_foto', $noticia->pie_foto) }}" placeholder="Texto descriptivo del archivo">
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="date" name="fecha_publicacion" class="form-control" value="{{ old('fecha_publicacion', $noticia->fecha_publicacion) }}" required>
                        </div>
                        <div class="form-group">
                            <label>Categoria <span style="color:#b91c1c">*</span></label>
                            <select name="categoria" class="form-control" required>
                                <option value="">Selecciona...</option>
                                <option value="Biodiversidad" {{ old('categoria', $noticia->categoria) == 'Biodiversidad' ? 'selected' : '' }}>Biodiversidad</option>
                                <option value="Cambio Climatico" {{ old('categoria', $noticia->categoria) == 'Cambio Climatico' ? 'selected' : '' }}>Cambio Climático</option>
                                <option value="Reciclaje" {{ old('categoria', $noticia->categoria) == 'Reciclaje' ? 'selected' : '' }}>Reciclaje</option>
                                <option value="Energia Renovable" {{ old('categoria', $noticia->categoria) == 'Energia Renovable' ? 'selected' : '' }}>Energía Renovable</option>
                                <option value="Conservacion" {{ old('categoria', $noticia->categoria) == 'Conservacion' ? 'selected' : '' }}>Conservación</option>
                                <option value="Educacion Ambiental" {{ old('categoria', $noticia->categoria) == 'Educacion Ambiental' ? 'selected' : '' }}>Educación Ambiental</option>
                                <option value="Agua" {{ old('categoria', $noticia->categoria) == 'Agua' ? 'selected' : '' }}>Agua</option>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="estado" class="form-control" required>
                                <option value="activa" {{ old('estado', $noticia->estado) == 'activa' ? 'selected' : '' }}>Activa</option>
                                <option value="inactiva" {{ old('estado', $noticia->estado) == 'inactiva' ? 'selected' : '' }}>Inactiva</option>
                            </select>
                        </div>
                    </div>
