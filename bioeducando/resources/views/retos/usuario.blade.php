...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        .container { padding: 40px 100px 40px 140px; flex: 1; width: 100%; margin: 0; }
        .retos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 30px; justify-content: center; }
        ...
        .modal-content { background-color: white; padding: 40px; border-radius: 30px; width: 90%; max-width: 500px; text-align: center; box-shadow: 0 25px 50px rgba(0,0,0,0.2); position: relative; border-top: 10px solid #6ab06a; }
        .modal-icon { width: 80px; height: 80px; background: #f0fdf4; color: #6ab06a; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; }
        .modal h3 { font-size: 1.8rem; color: #1a3a2a; margin-bottom: 15px; font-weight: 800; }
        .modal p { color: #64748b; margin-bottom: 30px; line-height: 1.6; }
        .progress-container { width: 100%; height: 12px; background: #e2e8f0; border-radius: 10px; margin-bottom: 10px; overflow: hidden; }
        .progress-bar { width: 0%; height: 100%; background: #6ab06a; border-radius: 10px; transition: width 1s ease-in-out; }
        .progress-text { font-weight: 700; color: #1a3a2a; margin-bottom: 30px; display: block; }
...
<body>
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="{{ route('retos.usuario') }}" class="menu-item active"><i data-lucide="leaf"></i> Retos Ecológicos</a>
            ...
            <div class="retos-grid">
                ...
                <div class="reto-card" onclick="window.location.href='{{ route('retos.seguimiento', $reto->id) }}'" style="cursor: pointer;">
                    ...
                    <div style="display: flex; justify-content: center; margin-top: 20px;">
                        <button class="btn-aceptar" onclick="event.stopPropagation(); mostrarProgreso('{{ $reto->titulo }}', {{ $reto->id }})">Dar inicio al reto <i data-lucide="play" size="16"></i></button>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div id="progresoModal" class="modal">
        <div class="modal-content"><i data-lucide="x" class="close-modal" onclick="cerrarModal()"></i><div class="modal-icon"><i data-lucide="trending-up" size="40"></i></div><h3 id="modalTitle">¡Misión Iniciada!</h3><p>Has comenzado este reto ecológico. ¡Sigue así para ganar tus puntos y ayudar al planeta!</p><div class="progress-container"><div id="progressBar" class="progress-bar"></div></div><span class="progress-text">Progreso: <span id="progressValue">0</span>%</span><button class="btn-entendido" onclick="irAlReto()">¡A por ello!</button></div>
    </div>
    <script>
        lucide.createIcons();
        let currentRetoId = null;
        function mostrarProgreso(titulo, id) { currentRetoId = id; const modal = document.getElementById('progresoModal'); const bar = document.getElementById('progressBar'); const val = document.getElementById('progressValue'); const title = document.getElementById('modalTitle'); title.innerText = "¡" + titulo + " Iniciado!"; modal.style.display = 'flex'; setTimeout(() => { bar.style.width = '15%'; val.innerText = '15'; }, 500); }
        function irAlReto() { if (currentRetoId) { window.location.href = "/mis-retos/" + currentRetoId; } }
        function cerrarModal() { const modal = document.getElementById('progresoModal'); const bar = document.getElementById('progressBar'); const val = document.getElementById('progressValue'); modal.style.display = 'none'; bar.style.width = '0%'; val.innerText = '0'; }
        window.onclick = function(event) { const modal = document.getElementById('progresoModal'); if (event.target == modal) { cerrarModal(); } }
    </script>
</body>
</html>
